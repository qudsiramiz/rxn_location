...
cone_angle_rx_en = (
    np.arccos(
        df_rx_en.b_imf_x / np.sqrt(df_rx_en.b_imf_x**2 + df_rx_en.b_imf_y**2 + df_rx_en.b_imf_z**2)
    )
    * 180
    / np.pi
)
cone_angle_va_cs = (
    np.arccos(
        df_va_cs.b_imf_x / np.sqrt(df_va_cs.b_imf_x**2 + df_va_cs.b_imf_y**2 + df_va_cs.b_imf_z**2)
    )
    * 180
    / np.pi
)
cone_angle_bisec = (
    np.arccos(
        df_bisec.b_imf_x / np.sqrt(df_bisec.b_imf_x**2 + df_bisec.b_imf_y**2 + df_bisec.b_imf_z**2)
    )
    * 180
    / np.pi
)
...
df_shear_lim = df_shear[
    (df_shear.b_imf_y.abs() / df_shear.b_imf_mag > 0.7) & (df_shear.b_imf_z > 0)
]
df_rx_en_lim = df_rx_en[
    (df_rx_en.b_imf_y.abs() / df_rx_en.b_imf_mag > 0.7) & (df_rx_en.b_imf_z > 0)
]
df_va_cs_lim = df_va_cs[
    (df_va_cs.b_imf_y.abs() / df_va_cs.b_imf_mag > 0.7) & (df_va_cs.b_imf_z > 0)
]
df_bisec_lim = df_bisec[
    (df_bisec.b_imf_y.abs() / df_bisec.b_imf_mag > 0.7) & (df_bisec.b_imf_z > 0)
]
...
for i in range(len(df_list)):
    print(
        f"{method_list[i]:<15}{df_list_lim[i].b_imf_x.median():<15.2f}{df_list_lim[i].b_imf_y.median():<15.2f}{df_list_lim[i].b_imf_z.median():<15.2f}{df_list_lim[i].b_imf_mag.median():<15.2f}{df_list_lim[i].p_dyn.median():<15.2f}"
    )
...
for i in range(len(df_list)):
    print(
        f"{method_list[i]:<15}{df_list[i].b_imf_x.median():<15.2f}{df_list[i].b_imf_y.median():<15.2f}{df_list[i].b_imf_z.median():<15.2f}{df_list[i].b_imf_mag.median():<15.2f}{df_list[i].p_dyn.median():<15.2f}"
    )
...
for i in range(len(df_list)):
    print(
        f"{'5 percentile':<15}{np.percentile(df_list[i].b_imf_x, 5):<15.2f}{np.percentile(df_list[i].b_imf_y, 5):<15.2f}{np.percentile(df_list[i].b_imf_z, 5):<15.2f}{np.percentile(df_list[i].b_imf_mag, 5):<15.2f}{np.percentile(df_list[i].p_dyn, 5):<15.2f}"
    )
    print(
        f"{method_list[i]:<15}{df_list[i].b_imf_x.median():<15.2f}{df_list[i].b_imf_y.median():<15.2f}{df_list[i].b_imf_z.median():<15.2f}{df_list[i].b_imf_mag.median():<15.2f}{df_list[i].p_dyn.median():<15.2f}"
    )
    print(
        f"{'95 percentile':<15}{np.percentile(df_list[i].b_imf_x, 95):<15.2f}{np.percentile(df_list[i].b_imf_y, 95):<15.2f}{np.percentile(df_list[i].b_imf_z, 95):<15.2f}{np.percentile(df_list[i].b_imf_mag, 95):<15.2f}{np.percentile(df_list[i].p_dyn, 95):<15.2f}"
    )
