...
try:
    ...
    st.error(f"Error importing rxn_location modules. Ensure the package is installed properly.\n{e}")
...
def reset_session():
    ...
    keys_to_clear = ["history", "crossing_time_str", "dark_mode", "initialized", "jet_view_idx", "rxn_view_idx", "latest_stats_csv"]
...
def main():
    ...
    if "initialized" not in st.session_state:
        ...
        if saved_state:
            st.session_state["history"] = saved_state.get("history", {"jet_checks": [], "recon_models": []})
            st.session_state["crossing_time_str"] = saved_state.get("crossing_time_str", "2015-09-02 16:45:00")
    ...
    with st.sidebar.expander("Session Manager", expanded=False):
        uploaded_file = st.file_uploader("Restore Session (.rxn_session)", type=["rxn_session", "pkl"])
        if uploaded_file is not None:
            if st.button("Load Session", use_container_width=True):
                try:
                    loaded_state = pickle.load(uploaded_file)
                    st.session_state["history"] = loaded_state.get("history", {"jet_checks": [], "recon_models": []})
                    st.session_state["crossing_time_str"] = loaded_state.get("crossing_time_str", "2015-09-02 16:45:00")
    ...
    show_hints = st.sidebar.checkbox("Show Parameter Hints", value=False, help="Toggle to show tooltip hints on optional parameters.")

    st.sidebar.header("Observation Parameters")
    
    date_str = st.sidebar.text_input("Crossing Time (UTC)", value=st.session_state.crossing_time_str)
    try:
    ...
    mms_probe = st.sidebar.selectbox("MMS Probe", [1, 2, 3, 4], index=2, help="Select which MMS spacecraft probe to use for the observation data." if show_hints else None)
    
    st.sidebar.markdown("---")
    st.sidebar.header("Statistics Mode Parameters")
    use_stats_mode = st.sidebar.checkbox("Enable Statistics Mode")
    stop_condition = st.sidebar.radio("Stop Condition", ["End Time", "Target Number of Jets"], disabled=not use_stats_mode)
    
    if stop_condition == "End Time":
        stats_end_time_str = st.sidebar.text_input("End Time (UTC)", value=st.session_state.crossing_time_str, help="Only used if Statistics Mode is enabled." if show_hints else None, disabled=not use_stats_mode)
        try:
            stats_end_time = datetime.datetime.strptime(stats_end_time_str, "%Y-%m-%d %H:%M:%S").replace(tzinfo=pytz.utc)
        except:
            stats_end_time = crossing_time
        target_jets = None
        end_str_safe = stats_end_time_str.replace(" ", "_").replace(":", "")
    else:
        target_jets = st.sidebar.number_input("Number of Jets to Find", min_value=1, value=10, disabled=not use_stats_mode)
        stats_end_time = None
        end_str_safe = f"{target_jets}jets"
    
    stats_delta_mins = st.sidebar.number_input("Time Delta (minutes)", min_value=1, value=15, help="Step size for the batch loop." if show_hints else None, disabled=not use_stats_mode)
    ...
    stats_csv_name = st.sidebar.text_input("Output CSV Name", value=default_csv_name, help="Name of the file to save batch data to." if show_hints else None, disabled=not use_stats_mode)
    
    sidebar_actions = st.sidebar.container() # placeholder for buttons
    
    # Create Tabs for the main display area
    if use_stats_mode:
        tab_controls, tab_jet, tab_rxn, tab_stats = st.tabs(["Controls", "Jet Reversal Plot", "Reconnection Models", "Statistics Results"])
    else:
        tab_controls, tab_jet, tab_rxn = st.tabs(["Controls", "Jet Reversal Plot", "Reconnection Models"])
        tab_stats = None
    ...
    with tab_controls:
        ...
        if len(st.session_state["history"]["jet_checks"]) > 0:
            ...
            if latest_jet["jet_detection"]:
                st.success(f"Latest Jet Check ({latest_jet['crossing_time']}): ✅ Jet Reversal Detected!")
            else:
                st.error(f"Latest Jet Check ({latest_jet['crossing_time']}): ❌ No Jet Reversal Detected.")
        ...
        with st.expander("Jet Reversal Advanced Parameters", expanded=True):
            ...
            dt = col1.number_input("Time Window (dt) [s]", value=300, help="Time window around the crossing time to search for jets (in seconds)." if show_hints else None)
            jet_len = col2.number_input("Jet Length Threshold", value=3, help="Minimum number of data points required to qualify as a valid jet." if show_hints else None)
            data_rate = col3.selectbox("Data Rate", ["brst", "fast", "srvy"], index=0, help="Data rate resolution of the MMS instruments." if show_hints else None)
            
            level = col1.selectbox("Data Level", ["l2", "l1"], index=0, help="Processing level of the MMS data (L2 is recommended for science analysis)." if show_hints else None)
            coord_type = col2.selectbox("Coordinate Type", ["lmn", "gse"], index=0, help="Coordinate system used for the magnetic field and velocity vectors." if show_hints else None)
            time_clip = col3.checkbox("Time Clip", value=True, help="Whether to strictly clip the data to the provided time window." if show_hints else None)

            t_delta = col1.number_input("Find Next Jet Time Delta (mins)", value=10, help="Time interval (in minutes) to step forward when searching for the next jet." if show_hints else None)
            max_attempts = col2.number_input("Max Attempts for Next Jet", value=5, min_value=1, help="Maximum number of time intervals to check before giving up." if show_hints else None)
        ...
        with st.expander("Reconnection Model Parameters", expanded=True):
            ...
            tsy_model = col1.selectbox("Tsyganenko Model", ["t89", "t96", "t01", "t04s"], index=1, help="Empirical magnetic field model used for the background magnetosphere." if show_hints else None)
            recon_models = col2.multiselect("Reconnection Models", ["shear", "bisection", "reconnection energy", "exhaust velocity"], default=["shear", "bisection", "reconnection energy", "exhaust velocity"], help="Physics models to calculate the probability of magnetic reconnection." if show_hints else None)
            
            omni_level = col1.selectbox("OMNI Data Level", ["hro_1min", "hro_5min"], index=0, help="Resolution of the OMNI solar wind data to pull for model parameters." if show_hints else None)
            m_p = col2.number_input("Proton Mass Multiple (m_p)", value=1.0, help="Scaling factor for the proton mass in calculations." if show_hints else None)
            dr = col1.number_input("Grid Resolution (dr)", value=0.5, help="Spatial resolution of the plotting grid in Earth Radii (RE)." if show_hints else None)
            limits = col2.number_input("Grid Limits (±)", value=20.0, help="Maximum extent of the X and Y plotting bounds (in RE)." if show_hints else None)
        ...
        def run_jet_check(c_time):
            ...
            """
            with st.spinner(f"Running Jet Reversal Check for MMS{mms_probe} at {c_time.strftime('%H:%M:%S')}..."):
                try:
                    ...
                    if res is None:
                        st.error(f"Magnetosphere or magnetosheath region not found for date {c_time.strftime('%Y-%m-%d %H:%M:%S+00:00')}")
                        return False, False
        ...
        def run_recon_models(c_time, save_data=False, csv_name="", det=None):
            ...
            if len(recon_models) == 0:
                ...
                return False
            with st.spinner(f"Calculating Reconnection Models for MMS{mms_probe} at {c_time.strftime('%H:%M:%S')} (this may take a while)..."):
                try:
                    model_mapping = {
    ...
    with sidebar_actions:
        ...
        if st.button("Find Next Jet", width="stretch"):
            if crossing_time:
                ...
                for attempt in range(max_attempts):
                    ...
                    st.session_state.crossing_time_str = current_test_time.strftime("%Y-%m-%d %H:%M:%S")
                    
                    if success and jet_det:
                        st.success(f"Jet found at {current_test_time.strftime('%Y-%m-%d %H:%M:%S')}!")
                        found = True
                        break
                
                if not found:
                    st.warning(f"No jet found after {max_attempts} attempts. Stopped at {current_test_time.strftime('%Y-%m-%d %H:%M:%S')}")
                save_auto_session()
                st.rerun()
            else:
                st.error("Invalid crossing time.")
        ...
        if st.button("Run Statistics Mode", width="stretch", disabled=not use_stats_mode):
            if not use_stats_mode:
                ...
                if stop_condition == "End Time" and stats_end_time and crossing_time > stats_end_time:
                    ...
                    if stop_condition == "End Time":
                        total_steps = int((stats_end_time - crossing_time).total_seconds() // (stats_delta_mins * 60)) + 1
                    else:
                    ...
                    while True:
                        ...
                        if stop_condition == "Target Number of Jets" and step > target_jets * 100: 
                            st.warning("Reached maximum search attempts (100x target). Stopping early.")
                            break
                            
                        if stop_condition == "End Time":
                            status_text.text(f"Processing {curr_t.strftime('%Y-%m-%d %H:%M:%S')} (Step {step+1}/{total_steps})")
                            top_counter_placeholder.markdown(f"<h1 style='text-align: center; color: #4CAF50;'>Jets Found: {jets_found}</h1>", unsafe_allow_html=True)
                        else:
                            status_text.text(f"Processing {curr_t.strftime('%Y-%m-%d %H:%M:%S')} (Found {jets_found}/{target_jets})")
                            top_counter_placeholder.markdown(f"<h1 style='text-align: center; color: #4CAF50;'>Jets Found: {jets_found} / {target_jets}</h1>", unsafe_allow_html=True)
                            
                        s1, det = run_jet_check(curr_t)
                        
                        if len(st.session_state["history"]["jet_checks"]) > 0:
                            latest_jet_fig = st.session_state["history"]["jet_checks"][-1].get("fig")
                            if latest_jet_fig:
                                with live_jet_plot_placeholder.container():
                                    st.plotly_chart(latest_jet_fig, use_container_width=True, key=f"live_jet_{step}")
                        
                        if s1 and det:
                            run_recon_models(curr_t, save_data=True, csv_name=stats_csv_name, det=det)
                            jets_found += 1
                    ...
                    if stop_condition == "End Time":
                        top_counter_placeholder.markdown(f"<h1 style='text-align: center; color: #4CAF50;'>Statistics Mode Completed! Total Jets Found: {jets_found}</h1>", unsafe_allow_html=True)
                    else:
                        top_counter_placeholder.markdown(f"<h1 style='text-align: center; color: #4CAF50;'>Statistics Mode Completed! Found {jets_found} / {target_jets} Jets.</h1>", unsafe_allow_html=True)
    ...
    with tab_jet:
        ...
        if len(jet_hist) > 0:
            ...
            if curr_idx < 0: curr_idx = len(jet_hist) - 1
            if curr_idx >= len(jet_hist): curr_idx = len(jet_hist) - 1
            ...
            if col_prev.button("Previous Run", key="prev_jet", width="stretch") and curr_idx > 0:
                new_idx = curr_idx - 1
            if col_next.button("Next Run", key="next_jet", width="stretch") and curr_idx < len(jet_hist) - 1:
                new_idx = curr_idx + 1
            ...
            st.write(f"**Parameters used:** Probe {active_run['mms_probe']}, dt {active_run['dt']}, jet len {active_run['jet_len']}, rate {active_run['data_rate']}, level {active_run['level']}, type {active_run['coord_type']}, clip {active_run['time_clip']}")
    ...
    with tab_rxn:
        ...
        if len(rxn_hist) > 0:
            ...
            if curr_idx < 0: curr_idx = len(rxn_hist) - 1
            if curr_idx >= len(rxn_hist): curr_idx = len(rxn_hist) - 1
            
            col_sel, col_prev, col_next = st.columns([6, 2, 2])
            selected_option = col_sel.selectbox("Browse Run History", options, index=curr_idx, key="sel_rxn")
            new_idx = options.index(selected_option)
            
            if col_prev.button("Previous Run", key="prev_rxn", width="stretch") and curr_idx > 0:
                new_idx = curr_idx - 1
            if col_next.button("Next Run", key="next_rxn", width="stretch") and curr_idx < len(rxn_hist) - 1:
                new_idx = curr_idx + 1
            ...
            matching_jet = next((j for j in st.session_state["history"]["jet_checks"] if j["crossing_time"] == active_run["crossing_time"]), None)
            ...
            st.write(f"**Parameters used:** Probe {active_run['mms_probe']}, Model {active_run['tsy_model']}, m_p {active_run['m_p']}, dr {active_run['dr']}, limit ±{active_run['limits']}, OMNI {active_run['omni_level']}")
    ...
    if tab_stats is not None:
        with tab_stats:
            ...
            csv_source = st.radio("CSV Source", ["Use generated from Statistics Mode", "Upload existing CSV"])
            ...
            if csv_source == "Use generated from Statistics Mode":
                if os.path.exists(default_csv):
                    ...
                    with open(csv_path, "rb") as f:
                        st.download_button("Download CSV", data=f, file_name=default_csv, mime="text/csv")
                else:
            ...
            if csv_path and os.path.exists(csv_path):
                ...
                plots_to_gen = st.multiselect("Select Figures to Generate", ["Histograms", "KDE Plots", "2D Histograms", "Scatter Plots", "MMS Location Scatter Plot"], default=["Histograms", "KDE Plots", "2D Histograms", "Scatter Plots"])
                
                x_vars = st.multiselect("X-Axis Variable(s)", list(var_options.keys()), format_func=lambda x: var_options[x], default=["b_imf_z"])
                y_vars = st.multiselect("Y-Axis Variable(s)", list(var_options.keys()), format_func=lambda x: var_options[x], default=["r_rc"])
                ...
                if st.button("Generate Selected Figures"):
                    with st.spinner("Generating statistical figures..."):
                        if not x_vars or not y_vars:
                            ...
                            st.error(f"Please select the same number of X and Y variables for pairwise plotting (you selected {len(x_vars)} X and {len(y_vars)} Y).")
                        else:
                            for x_var, y_var in zip(x_vars, y_vars):
                                if len(x_vars) > 1:
                                    st.markdown(f"### {var_options[x_var]} vs {var_options[y_var]}")
                                    
                                figures, err = generate_statistics_plots(csv_path, dark_mode=is_dark_mode, selected_plots=plots_to_gen, x_var=x_var, y_var=y_var)
                                if err:
                                    st.error(err)
                                else:
