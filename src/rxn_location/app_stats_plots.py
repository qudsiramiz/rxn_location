...
def generate_statistics_plots(
    csv_file_path, dark_mode=False, selected_plots=["Histograms"], x_var="b_imf_z", y_var="r_rc"
):
    figures = {}
    ...
    if any(
        p in selected_plots
        for p in ["KDE Plots", "2D Histograms", "Scatter Plots", "MMS Location Scatter Plot"]
    ):
        ...
        for df_n in [df_shear, df_rx_en, df_va_cs, df_bisec]:
            if len(df_n) > 0:
                if "imf_clock_angle" in df_n.columns:
                    df_n["imf_clock_angle"] = df_n["imf_clock_angle"].apply(
                ...
                if all(col in df_n.columns for col in ["b_imf_x", "b_imf_y", "b_imf_z"]):
                    ...
                    df_n["bb"] = df_n.b_imf_y / np.sqrt(
                        df_n.b_imf_x**2 + df_n.b_imf_y**2 + df_n.b_imf_z**2
                    )
            ...
            try:
                ...
                all_y = np.concatenate(
                    [
                        df_n[y_var].values
                        for df_n in df_list
                        if len(df_n) > 0 and y_var in df_n.columns
                    ]
                )
                ...
                if len(all_x) > 0 and len(all_y) > 0:
                    ...
                    for i, df_n in enumerate(df_list):
                        ...
                        for ii, x in enumerate(x_vals):
                            for j, y in enumerate(y_vals):
                                ...
                                if "r_rc" in subset.columns:
                                    z_vals[j, ii] = (
                                        np.nanmean(subset["r_rc"].values) if len(subset) > 0 else 0
                                    )
                                else:
                        ...
                        ax = axs[i // 2, i % 2]
                        im = ax.pcolormesh(
                            x_mesh, y_mesh, z_vals, cmap=cmap_list[i], shading="auto"
                        )
                        ...
                        ax.text(
                            ...
                            ha="right",
                            bbox=dict(
                                facecolor="white" if not dark_mode else "black",
                                alpha=1,
                        ...
                        cbar_label = (
                            r"Reconnection Distance $\left[R_{\rm E} \right]$"
                            if "r_rc" in df_n.columns
                            else "Count"
                        )
                        cbar.set_label(
                            cbar_label,
            ...
            try:
                ...
                for i, df_n in enumerate(df_list):
                    if len(df_n) == 0:
                        continue
                    if (
                        "spc_pos_x" in df_n.columns
                        and "spc_pos_y" in df_n.columns
                        and "spc_pos_z" in df_n.columns
                    ):
                        axs[0].scatter(
                            df_n["spc_pos_x"],
                            df_n["spc_pos_y"],
