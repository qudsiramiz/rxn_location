    ...
    for i, key in enumerate(keys_to_plot):
        ...
        for var_name in vars_to_plot:
            ...
            if hasattr(data, 'v') or hasattr(data, 'v1') or hasattr(data, 'v2'):
                v_data = data.v if hasattr(data, 'v') else (data.v1 if hasattr(data, 'v1') else data.v2)
                y_data = data.y
                ...
                fig.add_trace(
                    go.Heatmap(
                        ...
                        colorscale='Viridis',
                        showscale=True,
                        colorbar=dict(title=z_title, len=1.0/num_plots, y=y_center, yanchor='middle', x=1.02)
                    ),
                    row=i+1, col=1
                )
                ...
                if y_data.ndim == 1:
                    name = format_latex(legend_names[legend_idx] if legend_idx < len(legend_names) else var_name)
                    legend_idx += 1
                    fig.add_trace(
                    ...
                    for col in range(y_data.shape[1]):
                        name = format_latex(legend_names[legend_idx] if legend_idx < len(legend_names) else f"{var_name}_{col}")
                        legend_idx += 1
                        fig.add_trace(
                            go.Scatter(x=times, y=y_data[:, col], mode='lines', name=name, legend=legend_id),
                            row=i+1, col=1
                        )
                        
        if y_type == "log":
            fig.update_yaxes(type=y_type, title_text=y_title, exponentformat="power", dtick=1, row=i+1, col=1)
        else:
